...
DFS might not find the shortest path due to memorized property of DFS. But it could be possible if we run DFS for the whole graph and update the latest value (sounds like Dijkstra) -> this seems to be applied only when we tryna find something if it exists :) 
...
for neighbor in node.neighbors:
    if neighbor in parent and neighbor is not in finished:
